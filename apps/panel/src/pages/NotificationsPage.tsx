...
export function NotificationsPage() {
  ...
  async function save(t: NotifTemplate) {
    ...
    try { await notifApi.update(t.key, { channel: t.channel, subject: d.subject, body: d.body }); setMsg(`Template "${t.label}" tersimpan.`); qc.invalidateQueries({ queryKey: ["notif-templates"] }); }
  ...
  return (
    <section className="space-y-3">
      ...
      <p className="text-sm text-slate-500">Dikirim MANUAL dari halaman booking (tidak ada pengiriman otomatis). Placeholder: <span className="font-mono text-xs">{PLACEHOLDERS.join(" ")}</span></p>
      ...
      {q.data.map((t) => (
        <div key={t.key} className="rounded-xl border border-slate-200 bg-white p-4">
          <div className="mb-1 text-sm font-bold text-slate-700">{t.label} <span className="text-xs font-normal text-slate-400">· {t.channel === "wa" ? "WhatsApp" : "Email"}</span></div>
          <input className="mb-1 w-full rounded border border-slate-300 px-2 py-1 text-sm" placeholder="Subjek" value={drafts[t.key]?.subject ?? ""} onChange={(e) => setDrafts({ ...drafts, [t.key]: { ...drafts[t.key]!, subject: e.target.value } })} disabled={!canWrite} />
